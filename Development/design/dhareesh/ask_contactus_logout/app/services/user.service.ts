 ...
 isLogged(): Promise<boolean> {
  ...
  deleteEmployee(emailId) {
   ...
   contactUser(data:any){
      ...
      var headers = new Headers();
      headers.append('Content-Type', 'application/json');
      return this.http.post('http://192.168.1.111:8082/DemoAPI/rest/contact' , body,
       { headers: headers }).map((data: Response) => data.json());
   }
   addEmployee(model) {
    console.log(model);
    return this.http.post('http://192.168.1.106:8080/DemoAPI/rest/admin/registerEmployee', model
      ).map(res => res.json());
  }
  
  getEmp(){
    return this.http.get('http://192.168.1.106:8080/DemoAPI/rest/employee').map(res => res.json());
  }
  
  deleteEmp(emailId: any){
    return this.http.delete('http://192.168.1.106:8080/DemoAPI/rest/deleteemp' + emailId)
      .map(res => res.json());
  }
